...
const repo = process.env.GITHUB_REPOSITORY || 'seq23/approvalprep';
...
fs.writeFileSync('data/admin/content_queue_index.json', JSON.stringify({schemaVersion:'4.3.1', repo, branch, items, contractMode:'production_ready_links'},null,2)+'\n');
fs.writeFileSync('data/admin/admin_action_registry.json', JSON.stringify({schemaVersion:'4.3.1', actions:['open_github_edit','open_history','change_publish_date','copy_approval_instructions'], realBackendWriteback:false, items:items.map(i=>({id:i.id,title:i.title,editUrl:i.githubEditUrl,historyUrl:i.githubHistoryUrl}))},null,2)+'\n');
