...
//
// The defect this exists to prevent: across the portfolio, one field named `volume` held
// TWO incompatible quantities -- modelled monthly search volume on semrush_keyword_magic
// rows, and a domain's OWN 90-day impression count on gsc_search_analytics rows.
// rank_score was computed straight off it, so the two units were compared as peers and
// the content queue could invert.
//
// This repo has no GSC lane: all evidence rows are semrush_keyword_magic and there is no
// impressions field or GSC ingest script, so no row here was ever corrupted. The schema is
// split anyway, and this gate exists so the defect cannot appear the day a GSC lane lands.
//
...
const SCAN_DIRS = ['scripts', 'src', 'functions', 'ops', 'lib', 'tools', '_ops'];
