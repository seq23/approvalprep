...
// --accept records the current URL set as the new baseline. It requires a reason, so
// the record says why an overage was accepted rather than that someone re-ran the gate.
...
// never write it. It used to be written on every run -- the comment here argued the
// ledger records what exists rather than rewarding a pass, which is true of a ledger
// and fatal for a gate: the check consumed its own evidence, so any block cleared
// itself on the very next run with no change to the tree. Measured: first run exit 1,
// second run exit 0, identical tree.
//
