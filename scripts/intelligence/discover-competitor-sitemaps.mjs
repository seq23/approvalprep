...
import {readJson,writeJson,appendRun,now,checkBudget} from "./_lib.mjs";
const connectorId="competitor_sitemap_discovery";
const budget=checkBudget(connectorId);
if(!budget.allowed){ appendRun(connectorId,"BUDGET_HELD",{reason:budget.reason,recordsImported:0}); console.log(JSON.stringify({connectorId,status:"BUDGET_HELD",reason:budget.reason},null,2)); process.exit(0); }
const watch=readJson("data/intelligence/competitor_watchlist.json",{competitors:[]}).competitors.filter(c=>c.allowed); const deltas=[];
