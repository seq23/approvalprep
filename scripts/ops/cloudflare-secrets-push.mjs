...
if (!cloudflareApiToken) {
  throw new Error("CLOUDFLARE_API_TOKEN must be exported in the current shell before pushing secrets");
}

const { env } = await unlockVaultEnv({ real: true });
const wranglerEnv = { ...process.env, CLOUDFLARE_API_TOKEN: cloudflareApiToken };
...
for (const key of keys) {
  ...
  const child = spawnSync("npx", ["wrangler", "pages", "secret", "put", key, "--project-name", env.CLOUDFLARE_PAGES_PROJECT], { input: String(env[key]), stdio: ["pipe", "inherit", "inherit"], shell: false, env: wranglerEnv });
