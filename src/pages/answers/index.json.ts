...
export const GET: APIRoute = async () => new Response(JSON.stringify({ brand: "ApprovalPrep", boundary: "ApprovalPrep is self-service only. It does not repair credit, contact third parties, create fake documents, give legal or financial advice, or promise approval.", generatedAnswers: generated.answers.map((answer) => ({ ...answer, slug: slugify(answer.title), url: answer.status === "published_by_contract" ? canonicalUrl(`/blog/${slugify(answer.title)}`) : null })), atoms: atoms.atoms, claims: claims.claims }, null, 2), { headers: { "content-type": "application/json" } });
